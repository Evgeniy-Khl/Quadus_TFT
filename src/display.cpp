...
const char* labelsMenu1[POINTS_1] = {
  "увімкн. °C",    "вимкн. °C", 
  "гістерезис °C", "аварійне °C", 
  "заслінка полож.",  "програма",
  "ВІХІД", "наступне"
};
...
//-----------
  #define posTxt 0
  #define posErr 160
  tft.setTextPadding(310);  // задает ширину области автоматической очистки (заливки фоном) при выводе текста
  //------------ ОСВІТЛЕННЯ ----------
  ypos = h+8-5;
  ...
  if (PCF_ERR){
    ...
    tft.drawString("    Фатальна помилка керування!", posTxt, ypos);
  } else {
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    tft.drawString("ОСВІТЛЕННЯ:", posTxt, ypos);
  }
  ...
  if (!RTCENABLE){
    tft.setTextColor(TFT_RED, TFT_BLACK);
    tft.drawString("Немає годинника", posTxt, ypos);
  } else {
    sprintf(displStr,"Уві.%02d Вим.%02d", settings.timerOn, settings.timerOff);
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
    tft.drawString(displStr, posTxt, ypos);
  }
  ...
  if (manual_control){
    tft.setTextColor(TFT_YELLOW, TFT_RED);
    tft.drawString("Ручне керування!", posTxt, ypos);
  } else {
  ...
  if (!WIFIENABLE){
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    tft.drawString("Немає мережі!", posErr, ypos);
  } 
  ...
  for (uint8_t row = 0; row < amt_row; row++) {
    if(row < amt_row-1){
      for (uint8_t col = 0; col < amt_col; col++) {
        ...
        key[b].setLabelDatum(0, 6); // опускаем текст на 3 пикселя
...
uint16_t lampUpdate(uint16_t xpos, uint16_t ypos){
    ...
    if (settings.modeHeater == 0){
      if(on) tft.setTextColor(TFT_BLACK, TFT_ORANGE, true);
      else tft.setTextColor(TFT_BLACK, TFT_BLACK, true);
      tft.print(" НАГРІВ ");
      txt_width = tft.textWidth(" НАГРІВ ");
    } else {
      if(on) tft.setTextColor(TFT_WHITE, TFT_BLUE, true);
      else tft.setTextColor(TFT_BLACK, TFT_BLACK, true);
      tft.print(" ОХОЛОД ");
      txt_width = tft.textWidth(" ОХОЛОД ");
    }
    ...
    if (settings.modeHumidi == 0){
      ...
      tft.print(" ОСУШЕН ");
      txt_width = tft.textWidth(" ОСУШЕН ");
    }
